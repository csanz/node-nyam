...
var fs = require('fs')
  , noop = function(){}
  , utils = require('./utils')
  , PATH_CONFIG = __dirname + '/config.json';
...
function Config() {
	...
	utils.file_exists(false,process.env.HOME + '/.nyam_keys', function(success, file, err){
		...
		} else { keys = JSON.parse(fs.readFileSync('./nyam_keys',"utf8"));}
	});
	...
	this.override_status = (override) ? '\nUsing override config at ~/.nyam_keys'.blue : 'Using default keys config'.green;
